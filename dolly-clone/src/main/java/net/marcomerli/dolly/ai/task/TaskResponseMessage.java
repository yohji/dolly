...
import net.marcomerli.dolly.ai.action.ActionResponseMessage;
...
public class TaskResponseMessage extends Task {
	...
	public void init() throws Exception
	{
		...
		if ( store.hasMessages() ) {
			message = store.values().get( 0 );
			store.remove( message.uid(), message );
		}
	...
	{
		register( new ActionResponseMessage( message ) );
	}
